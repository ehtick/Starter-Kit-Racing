...
export class LapTimer {
	...
	constructor( cells, trackId ) {
		...
		this.lineHalfWidth = CELL_RAW * GRID_SCALE * 0.5;
	...
	update( dt, position, hasInput ) {
		...
		if ( this.prevForwardProj !== null ) {
			...
			const crossed =
				this.prevForwardProj < 0 &&
				forwardProj >= 0 &&
				lateralProj <= this.lineHalfWidth &&
				Math.abs( forwardProj - this.prevForwardProj ) < 5;

			if ( crossed ) this.completeLap();
